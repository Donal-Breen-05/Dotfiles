...
sync_dir() {
  ...
  if rsync --exclude-from={'list.txt'} "${RSYNC_OPTS[@]}" \
    "${REMOTE_USER}@${TARGET_HOST}:${remote_path}" \
    "${local_path}"; then
...
# Run all syncs
sync_dir "CODE" "$REMOTE_CODE" "$LOCAL_CODE"
sync_dir "NOTES" "$REMOTE_NOTES" "$LOCAL_NOTES"
sync_dir "SCRIPTS" "$REMOTE_SCRIPTS" "$LOCAL_SCRIPTS"
sync_dir "PERSONAL" "$REMOTE_PERSONAL" "$LOCAL_PERSONAL"
